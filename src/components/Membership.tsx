...
    if (loading) return <p className="text-center text-muted-foreground">Yükleniyor...</p>;
    if (filteredPlans.length === 0) return <p className="text-center text-muted-foreground">Plan bulunamadı.</p>;
...
            <Card
              key={index}
              className={`relative bg-card/50 backdrop-blur-sm border-border hover:border-${color}/50 transition-all duration-300 hover:scale-105 ${isPopular ? "ring-2 ring-primary/50" : ""} flex flex-col`}
            >
              {isPopular && (
...
        {/* Tabs: vertical on mobile, horizontal on md+; clear separation from content below */}
...
          {/* separation: bigger margin on desktop too */}
