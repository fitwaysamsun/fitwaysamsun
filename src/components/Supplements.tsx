        ...
        const results = await Promise.all(
          categories.map(async (category) => {
            ...
            const product: ContentfulProduct = {
              title: entry.fields.name || "Ürün Adı Yok",
              price: entry.fields.price ? Number(entry.fields.price) : null,
              description: entry.fields.description || "",
              image:
                entry.fields.photos &&
                entry.fields.photos.length > 0 &&
                entry.fields.photos[0].fields?.file?.url
                  ? `https:${entry.fields.photos[0].fields.file.url}`
                  : ""
            };
